...
// The "public" function
function indexToLabelFromSeed(seed) {
...
function labelToIndexFromSeed(seed) {
	var pair = indexToLabelFromSeed(seed);
	var homeIndices = pair[0];
	var awayIndices = pair[1];
	return [_dirtyInvert(homeIndices), _dirtyInvert(awayIndices)];
}
export { indexToLabelFromSeed, labelToIndexFromSeed };
